...
def fetch_matches(match):
    ...
    try:            
        ...
        if len(data) == 1 and data == [2]:
            obj.to_sql('NONEXISTENT_FAILS_TEMP',con=engine, if_exists="append",index=False)

        elif len(data) == 1 and data == [1]:
            obj.to_sql('TIMEOUT_FAILS_TEMP',con=engine, if_exists="append",index=False)

        elif len(data) == 1 and data == [3]:
            obj.to_sql('NONAOE2MATCH_FAILS_TEMP',con=engine, if_exists="append",index=False)
            
        else:
            data.to_sql('RAW_MATCH_DATA_TEMP',con=engine, if_exists="append",index=False)
    except:
        obj.to_sql('UNKNOWN_FAILS_TEMP',con=engine, if_exists="append",index=False)
